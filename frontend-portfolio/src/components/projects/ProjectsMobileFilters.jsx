...
export default function ProjectsMobileFilters({ isFiltersOpen, setIsFiltersOpen }) {
  ...
  return (
    <div className="hidden max-[1060px]:flex flex-col">
      ...
      <button
        className="flex items-center justify-center w-full h-10 gap-2 p-2 transition-colors border-b cursor-pointer bg-bg-terminal border-border-ide text-text-default hover:text-text-selected group"
      ...
      {/* Filtres */}
      <div
        className={`absolute top-full left-0 right-0 text-text-default bg-bg-terminal border-b border-border-ide transition-all duration-300 overflow-hidden ${isFiltersOpen ? "max-h-120" : "max-h-0"
          }`}
      >
        {/* Plateformes */}
